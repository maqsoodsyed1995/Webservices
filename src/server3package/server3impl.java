	...
	
{
	...
	{	
	  ...
	  public  String  cancelBooking (String bookingID, String userid)
	  {
										...
										synchronized(this)
										{
				  						...
				  						System.out.println("Booking cancellation was successful");
												...
												{	
													...
													fwriter(s+"	"+s1+"	"+s2+" 	booking cancellation successful in WST","WSTSERVER");
												} 
												catch (IOException e) 
												{
													e.printStackTrace();
			...
			w1=w1.concat(w2);
			w1=w1.concat(w3);
			System.out.println(w1+"    CONCATINATED STRING ");
			...
			if(con.equals(new String("cancelled")))
			{
				try 
				{
					...
					fwriter(bookingID+"	(Cancelled booking)","WSTSERVER");
				} 
				catch (IOException e) 
				{
			...
			try 
			{
				...
				fwriter(bookingID+"	(cancellation was not succesful)","DVLSERVER");
			} 
			catch (IOException e) 
			{
		...
		while(ib1.hasNext())
		{
			...
			while(setn1.hasNext())
			{
				...
				{   
					...
					System.out.println("in if of size==3");
			        	   ...
			        	   try 
							{
								...
								fwriter(booking_id+"	change reservation was not successful as the new room the student waants to book is already booked","WSTSERVER");
							} 
							catch (IOException e) 
							{
			        	   ...
			        	   try 
							{
								...
								fwriter(new_date+"		"+new_campus_name+" 	"+new_room_no+" 		"+new_time_slot+"		"+ret+" 		"+"(reservation changed)","WSTSERVER");
							} 
							catch (IOException e) 
							{
				...
				{
					System.out.println("inside if of size not equals 3");
					ret=bookroom(new_campus_name,new_room_no, new_date,new_time_slot,studentid);
